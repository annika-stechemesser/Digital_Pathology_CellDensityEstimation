...
f=@entropyfilt;                                 %build a function handle, use entropy filter
...
figure;
subplot(1,3,1);imshow(mask); title('before pruning');
subplot(1,3,2); imshow(mask_prune);title('after morphologically opening');
subplot(1,3,3); imshow(mask_prune2);title('after morphologically opening and closingl');

%%

B = bwboundaries(mask_prune2/255,'noholes');
disp('Done entropyfilt')
save('./tissue_mask_entropy.mat','mask_prune2'); %save the binary mask for tissue segments

% Visualiztion of each step of segmentation process
figure(1); clf;
subplot(1,4,1);imshow(im);title('WSI at Level 3');
subplot(1,4,2);imagesc(features);colormap jet;truesize;title('Local entropy');
subplot(1,4,3);imshow(mask_prune2); title('Segmentation Mask entropyfilt');
subplot(1,4,4);imshow(im); title('Segmentation Boundary Overlay entropy');
hold on
for k = 1:length(B)
   boundary = B{k};
